...
class GenerateBimiController(Resource):
    ...
    def post(self):
        ...
        svg_generate_vmc = vmc_link
        if self.svg_file_flag:
            svg_generate_link = self.Utils.svg_replace_file_link(content['domain'], svg_link)
        # if self.vmc_file_flag:
        #     vmc_generate_link = vmc_replace_file_link()

        if (data['svg_validation']['status'] and data['dmarc']['status'] and data['mx']['status'] and data['spf']['status']):
            GB = GenerateBimi()
            data['bimi_generation'] = GB.generate_bimi(content['domain'], svg_generate_link, svg_generate_vmc)
            data['bimi_generation']['svg_link'] = svg_generate_link
        else:
            data['bimi_generation'] = {"status":False, "record":"", "message":"Some errors was found in your bimi check","errors":["Please fix the errors shown below to successfully generate BIMI record."], "svg_link":svg_generate_link}
